# Cherry Agent Security Policy

This document describes Cherry Agent's trust model, names the one
...
Cherry Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Cherry Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Cherry Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Cherry Agent's documentation
...
Cherry Agent supports two OS-level isolation postures. They address
...
Under a whole-process wrapper, Cherry Agent's in-process heuristics
...
Cherry Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Cherry Agent
itself. Bugs in Cherry Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Cherry Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Cherry Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Cherry Agent has documented a stance about how its output should
...
- Cherry Agent includes supply-chain guards for MCP server
